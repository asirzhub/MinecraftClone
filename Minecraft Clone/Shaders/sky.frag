...
void main()
{
    ...
    float cloudiness = pow(fbm((hit.xz + cameraPos.xz)/cloudScale, 5, 2.6, 0.6), 1.2) * bz;
    ...
    vec3 col = sky + sunGlow + sunDisc * sunColor + vec3(c4, c4/2, c4/3);
    FragColor = vec4(col + vec3(cloudiness) * h, 1.0);
}
